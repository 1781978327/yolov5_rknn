...
high_thresh = 0.6
match_thresh = 0.8
reid_model_path = model/osnet_x0_25_market.rknn
...
- `[tracker]` 是全局跟踪器默认值
- 每个 `[videoX]` / `[video]` 都可以单独覆盖：
  `tracker_enable`、`tracker_type`、`track_buffer`、`track_thresh`、`high_thresh`、`match_thresh`、
  `reid_model_path`、`reid_feature_dim`、`reid_cpu_id`、`reid_npu_core`
- `stream_name` 决定 RTSP 路径，例如 `cam0` 对应 `/cam0`
- 如果没有 `[video0]`、`[video1]`，程序会回退到旧的单路 `[video]` 配置
- `tracker_type` 支持 `none`、`bytetrack`、`deepsort`
- `deepsort` 需要额外的 ReID 模型，默认已附带 `model/osnet_x0_25_market.rknn`
- 开启跟踪后，叠字会显示 `ID:<track_id> class score`，并绘制轨迹线
