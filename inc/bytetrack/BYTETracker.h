...
public:
    BYTETracker(int frame_rate = 30, int track_buffer = 30,
                float track_thresh = 0.5, float high_thresh = 0.6, float match_thresh = 0.8);
    ~BYTETracker();

    vector<STrack> update(const vector<Object>& objects, int fps, long long num_frames);
    cv::Scalar get_color(int idx);

    void set_track_thresh(float t) { track_thresh = t; }
    void set_high_thresh(float t) { high_thresh = t; }
    void set_match_thresh(float t) { match_thresh = t; }
